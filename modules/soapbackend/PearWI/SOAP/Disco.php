...
class SOAP_DISCO_Server extends SOAP_Base_Object {
    ...
    function getDISCO($extra = '')
    {
        $this->_generate_DISCO($extra);
        return $this->disco;
    }
    ...
    function _generate_DISCO($extra = '')
    {
        // DISCO
        ...
        $this->_disco['disco:discovery']['scl:contractRef']['attr']['ref'] =
            ...
            ? 'https://' . $this->host . $_SERVER['PHP_SELF'] . '?wsdl' . $extra
            : 'http://'  . $this->host . $_SERVER['PHP_SELF'] . '?wsdl' . $extra;
